...
async function createWindow(): Promise<void> {
  ...
  const windowSettings: Electron.BrowserWindowConstructorOptions = {
    ...
    minimizable: false,
    maximizable: false,
    opacity: 0.2, // Reduced opacity for better transparency
    ...
    // Enable click-through for transparent areas
// transparent property already defined above, removing duplicate
  ...
  // Add this to prevent accidental closing
  // state.mainWindow.on('close', (event) => {
  //   if (state.processingHelper?.isProcessing()) {
